...
rec {
  ...
  loadOverlays =
    ...
    in
    map (src: import src.path { inherit inputs; }) sources;
  ...
  loadPackages =
    ...
    let
      sources = loadSources dir;
    in
    utils.listToNestedAttrs (p: pkgs.callPackage p.path { }) sources;
